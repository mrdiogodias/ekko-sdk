...
void RTC_Set_Time(uint8_t sec, uint8_t min, uint8_t hour)
{
	...
	set_time[4] = decToBcd(0);
	set_time[5] = decToBcd(0);
	set_time[6] = decToBcd(0);
	set_time[7] = decToBcd(0);

	I2C_Transmit(i2c, 0xD0, set_time, 8);
}
...
uint8_t RTC_Get_Seconds(void)
{
	...
	get_time[0] = 0x0;
	I2C_Receive(i2c, 0xD1, get_time);
	
	return bcdToDec(get_time[1]);
}
